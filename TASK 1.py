...
#  	This program will:
# 					Show the day of the week that you visited per store for week 1 and 2
...
def dataofshoppinglist():
    ...
    filepath = "DADSA CWK SHOPPING DATA WEEK 1 File A.csv"
    shopping_data_list = readcsv(filepath)
    for item, Item_of_product in enumerate(shopping_data_list):  # Loop through each row in store and the item of
        # product
...
# By getting the top number in the list, this will show what the best shop from also getting its index.

def discoverbeststore(product_of_items):
    store_points = [0] * len(Stores)

    for item in product_of_items:
        for i, store in enumerate(Stores):  # This will loop through the stores
            listed = store.availableitemsinstock(item[0])  # checks the item is available
            if listed:
                store_points[i] += 1
...
def bestalternatives(missed_product_of_items):
    alternative_items = []
    best_alternative = ""
    top_word_matches = 0

    for Missed_product_of_item in missed_product_of_items:
        for Product_of_items in Items:
            ...
            if len(matched_word) > top_word_matches and Missed_product_of_item[0] != Product_of_items:
                ...
                best_alternative = Product_of_items  # This wil will pick the best substitutes of the product of items
    ...
    for n in range(Number_of_weeks):
        for house in Houses:
            ...
            while len(Missed_Items) != 0:
                if count < maximum_days_between_deliveries:
